(() => {
  ...
  function updateParallax() {
    ...
    if (orbGold) orbGold.style.transform = `translate3d(0, ${y * 0.10}px, 0)`;
    if (orbSage) orbSage.style.transform = `translate3d(0, ${y * -0.14}px, 0)`;
    if (orbInk) orbInk.style.transform = `translate3d(0, ${y * 0.07}px, 0)`;
  }
  ...
  const revealItemGroups = document.querySelectorAll('.cert-grid, .timeline, .skills-columns, .log-list');
  ...
  if (orbitFrame && canHover && !reduceMotion) {
